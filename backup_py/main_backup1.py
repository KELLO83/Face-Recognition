...
def main():
    ...
    logging.info(f"GPU Available: {ngpus_per_node}")
...
def main_worker(rank, world_size):
    ...
    if opt.pretrained_weights and os.path.exists(opt.pretrained_weights):
        load_result = backbone.load_state_dict(
            torch.load(opt.pretrained_weights, map_location='cpu'), 
            strict=False
        )
        if rank == 0:
            logging.info("Pre-trained weights loaded.")
            if load_result.missing_keys or load_result.unexpected_keys:
                logging.info("Missing keys:", load_result.missing_keys)
                logging.info("Unexpected keys:", load_result.unexpected_keys)
    else:
        if rank == 0:
            logging.info("No pre-trained weights found or specified, starting from scratch.")
    ...
    model = torch.compile(model, options={"max_autotune": False})
    ...
    for i in range(opt.max_epoch):
        ...
        for ii, (data_input, label, _) in tqdm(enumerate(trainloader), total=len(trainloader), disable=(rank != 0)):
            ...
            if rank == 0 and (ii + 1) % opt.print_freq == 0:
                ...
                speed = (ii + 1) / elapsed  
                avg_loss = train_loss / ((ii + 1) * opt.train_batch_size)
                avg_acc = train_corrects / train_total
                print(f'Epoch {i}/{opt.max_epoch-1}, Iter {ii+1}/{len(trainloader)} | '
                      f'Train Loss: {avg_loss:.4f}, Acc: {avg_acc:.4f}, Speed: {speed:.2f} iters/s')
        ...
        if rank == 0 and (i % opt.save_interval == 0 or i == opt.max_epoch - 1):
