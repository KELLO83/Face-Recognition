...
LOG_FILE = os.path.join(script_dir, "single_insightface.txt")
    ...
    for img_path in tqdm(all_images, desc="임베딩 추출"):
        try:
            ...
            if len(faces) == 0:
                ...
                # 정규화된 임베딩(normed_embedding)을 사용해야 합니다.
...
def plot_roc_curve(fpr, tpr, roc_auc, model_name, excel_path):
    ...
    plt.title(f'ROC Curve for InsightFace {model_name}')
...
def main(args):
    ...
    # --- 3단계: InsightFace 모델 로드 ---
    print(f"\nInsightFace {args.model_name} 모델을 로드합니다...")
    
    model_info = {
        "antelopev2": "ResNet-100 + Glint360K (최고 성능)",
        "buffalo_l": "ResNet-50 + WebFace600K (높은 성능)", 
        "buffalo_m": "ResNet-50 + WebFace600K (중간 성능)",
        "buffalo_s": "MobileFaceNet + WebFace600K (빠른 속도)",
        "buffalo_sc": "MobileFaceNet + WebFace600K (초경량)"
    }
    print(f"모델 정보: {model_info.get(args.model_name, '정보 없음')}")
    
    face_app = FaceAnalysis(
        name=args.model_name,
        providers=["CUDAExecutionProvider", "CPUExecutionProvider"],
        root="." # insightface가 기본 경로(~/.insightface/models)를 잘 찾으므로 보통은 수정 불필요
    )
    ...
    if labels.size > 0:
        ...
        save_results_to_excel(excel_path, f"InsightFace_{args.model_name}", roc_auc, eer, tar_at_far_results, args.target_fars, metrics)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="/home/ubuntu/Downloads/mini_dataset", help="평가할 데이터셋의 루트 폴더")
    parser.add_argument("--model_name", type=str, default="antelopev2", 
                       choices=["antelopev2", "buffalo_l", "buffalo_m", "buffalo_s", "buffalo_sc"],
